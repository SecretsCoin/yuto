...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("YutoCoin"))
        return false;
...
{
    // Convert YutoCoin:// to YutoCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("YutoCoin://"))
    {
        uri.replace(0, 12, "YutoCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "YutoCoin.lnk";
}
...
{
    return GetAutostartDir() / "YutoCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("YutoCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  YutoCoin-qt [" + tr("command-line options") + "]                     " + "\n";
